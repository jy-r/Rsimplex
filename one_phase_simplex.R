...
(sol <- Rsimplex(A,b,C,log = FALSE))
(sol <- Rsimplex(A,b,C,log = TRUE))

Rsimplex <- function(A,b,C,log=TRUE){
  #check Input
  if(dim(A)[1]!=length(b)){print("Matrix A doesnt correspond to vector b")
    stop()}
  if(dim(A)[2]!=length(C)){print("Matrix A doesnt correspond to vector C")
    stop()}  
